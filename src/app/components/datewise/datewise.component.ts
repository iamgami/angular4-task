...
export class DatewiseComponent implements OnInit  {
  ...
  constructor(private router: Router, public _APIservices: Api, ) {
    ...
    this.countries = [];
    if(router.url == '/date-wise' )
    {
      this.parentPage = 'D2D Collection By Date';
  ...
  async doPlay() {
    ...
    for (let vehicle of this.getAllVehiclesList) {
      if(vehicleRTO == vehicle.rto_number) {
        ...
        var thisSet = this;
        setTimeout(function () {
          thisSet.onSelectZone(vehicle.zone_id);
          thisSet.onSelectWards(vehicle.ward_id);
          thisSet.getRegularFeed(vehicle);
  ...
  getRegularFeed(vehicle){
    ...
    this._APIservices.api_gps_playback(vehicle.gpsDevice.imei, body_param.start_date).subscribe(suc => {
