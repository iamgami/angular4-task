...
@Component({
  selector: 'app-twin-bin',
  templateUrl: './twin-bin.component.html',
  styleUrls: ['./twin-bin.component.scss']
})
export class TwinBinComponent implements OnInit  {
  loadjson: any;
  opensidebar: any;
  closesidebar: any;
  //Show Loader Status
  ...
  constructor(private router: Router, public _APIservices: Api, ) {
    ...
    if(router.url == '/twin-bin' )
    {
      this.parentPage = 'Twin Bin Collection';
        ...
        if(stopageKeyArray[indexTrack] != undefined) {
          ...
          if(stopageArray.duration >= 300) {
            ...
            if(indexTrack == stopageArray.end){
              console.log("Show Stop"+stopageKeyArray[indexTrack]);
